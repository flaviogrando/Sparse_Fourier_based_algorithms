...
% Análise do ponto fora da grade em função do kernel
% 
...
% Cria vetor com correções (para falha do kernel)
for i=1:1:20
    fail(i) = 10^(-i);
end

ff=-5:0.1:5; % cria vetor para deslocar o ponto da grade
...
errorGoal =  1e-15;
tic;
[spectro1, indice1]=OMP_1(Ddk, Sa.',m,0,errorGoal);
toc;
...
ylabel('Amplitude (pu)'), xlabel(' \delta''_{1} ')
subplot(2,1,2), hold on, grid on
...
%title('Fase (rad)')
%legend('DFT','R-Prony (ref)', 'R-Prony (dft)', 'MR-Prony (ref)', 'MR-Prony (dft)')
ylabel('Fase (rad)'), xlabel(' \delta''_{1} ')
...
ylabel('Erro de amplitude (%)'), xlabel(' \delta''_{1} ')
subplot(3,1,3), hold on, grid on
...
ylabel('Erro de fase (º)'), xlabel(' \delta''_{1} ')
