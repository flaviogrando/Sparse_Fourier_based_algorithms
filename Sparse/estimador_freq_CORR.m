...
function [Ssa, Asa, phisa, indx_a] = estimador_freq_CORR(Sa, K, P, omp, Fs, freq, Dic)
...
switch omp  % Seleciona  OMP
    case 1% (Phi,V,m,alpha,errorGoal)
        for i=1:num_win
            [Ssa(i,:), coef(i,:), indx_a(i,:), ra] = CORR_amp(K, Sa(i,:).', Dic);
    ...
    case 2  
        for i=1:num_win
            [Ssa(i,:), coef(i,:), indx_a(i,:), ra] = CORR_amp2(K, Sa(i,:).', Dic);
