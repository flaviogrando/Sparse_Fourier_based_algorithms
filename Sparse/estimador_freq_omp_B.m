...
function [Ssa, Asa, phisa, indx_a] = estimador_freq_omp_B(Sa, K, P, omp, Fs, f0, freq, block)
...
%[Dic] = dic_freq_seletor(N, H, Fs, f0, freq(j));
%load('E:\block_dic32us.mat') % carrega dicionários
%load('E:\block_dic256us.mat') % carrega dicionários
    ...
    switch omp  % Seleciona  OMP
        ...
        case 3
            for i=1:num_win     % OMP tradicional
                [Dic] = dic_freq_seletor(N, H, Fs, f0, freq(i), block);            % <<<<<<<<< SELEÇÃO DO DICIONÁRIO
        ...
        case 5
            for i=1:num_win
                [Dic] = dic_freq_seletor(N, H, Fs, f0, freq(i), block);
                [Ssa(i,:), coef(i,:), indx_a(i,:)] = OMP_freq_freqB(K, Sa(i,:).', Dic);
            end
    end
